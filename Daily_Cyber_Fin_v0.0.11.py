...
import json
import re
import time # 新增 time 模組用於分段發送延遲
...
def run_security_mode(config):
    ...
    # 1. 新聞快報 (TG) - 回歸 V0.0.1 的清爽風格 (無攻防細節，只給懶人包)
    print(" ↳ 發送新聞快報 (TG)...")
    news_prompt = f"""
    ...
    # 針對挑選出的新聞，生成一段「包含多則教學」的長文，我們在這邊一次生成，但結構上分開
    class_prompt = f"""
    ...
    # 這裡我們一次發送，因為 TG 對長文支援度還行，分太細反而會因為 API 限制被擋
    # 如果內容真的很長，AI 會自動分段，但我們這裡用一個長訊息發送比較保險
    send_telegram(token, model.generate_content(class_prompt).text, log_file=SECURITY_TG_LOG)
    
    # 3. MD 日報 (Log) - 保持 NotebookLM 專用結構 (詳盡版)
