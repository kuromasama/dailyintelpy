...
def run_security_mode(config):
    ...
    # 3. MD 日報 (Log) - 生動詳細版 (解決 CISO 太短問題)
    print(" ↳ 生成詳細 MD 日報...")
    file_prompt = f"""
    請撰寫一份 **生動且詳盡的資安戰情白皮書 (Markdown 格式)**。
    標題：# 🛡️ 資安戰情白皮書 ({today_str})
    【內容】{raw}
    
    【要求】
    1. **使用豐富的 Emoji** 讓版面生動 (如 🚨, 🎯, 💡)。
    2. **寫作風格**：像一篇精彩的技術部落格，深入淺出。
    
    【結構】
    ## 1. 👨‍💼 CISO 架構師總結 (Macro View)
    - **今日威脅綜述**：以故事性口吻總結今日局勢。
    - **一句話戰略建議**：給管理者的真心話。
    ...
    ## 3. 🎯 全面技術攻防演練 (Micro View)
    - **請針對今日「每一則」重要新聞，都撰寫一段攻防分析**。
    - 包含：**攻擊向量**、**緩解措施**、**名詞解釋**。
    
    ## 4. 🔮 威脅趨勢與未來預測 (Future Outlook)
    - **(請獨立成段，寫長一點)** 預測駭客下一步動向與新興手法。
    """
