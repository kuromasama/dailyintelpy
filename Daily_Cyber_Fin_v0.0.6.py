    ...
    for url in urls:
        try:
            ...
            for entry in feed.entries:
                ...
                # 這裡只提供標題與連結給 AI，讓 AI 自己去組織語言和日期
...
def run_security_mode(config):
    ...
    # 1. 新聞快報 (TG) - 回歸 V0.0.1 風格 (全中文翻譯，不保留英文標題)
    print(" ↳ 發送新聞快報...")
    news_prompt = f"""
    ...
    # 2. CISSP 深度技術課 (TG) - 針對「單一最重要事件」的硬核教學
    print(" ↳ 發送 CISSP 單點突破教學...")
    class_prompt = f"""
    ...
    # 3. MD 日報 (Log) - 包含「CISO 總結」與「所有事件的技術分析」
    print(" ↳ 生成詳細 MD 日報...")
    file_prompt = f"""
    請撰寫一份 **全方位的資安戰情白皮書 (Markdown 格式)**。
    ...
    ## 2. 🌍 全球威脅列表
    - 列出所有新聞 (含原始英文標題、中文翻譯、連結)。
    
    ## 3. 🎯 全面技術攻防演練 (Technical Deep Dive)
    - **請針對今日「每一則」重要新聞，都撰寫一段攻防分析** (這是重點！)。
    - 對於每則新聞，請列出：
        - **⚔️ 攻擊向量 (Attack Vector)**
        - **🛡️ 緩解措施 (Mitigation)**
        - **🧠 涉及的 CVE 或技術名詞解釋**
    
    (請使用豐富的 Markdown 排版，包含粗體、列表、代碼區塊)
    """
...
def run_finance_mode(pf_data, mode="finance"):
    ...
    # 3. MD 日報 (Log) - 包含 ASCII Art 與 深度診斷
    if mode == "finance":
        print(" ↳ 生成詳細 MD 日報...")
        file_prompt = f"""
        ...
        ## 2. 🕯️ K 線技術教學 (含圖解)
        - 請嘗試用 ASCII Art 畫出今日教學的 K 線型態 (例如紅K、十字線、W底)。
