...
def save_log(filename, content):
    # Log 檔案不需要加日期標題，因為 Prompt 裡面已經要求 AI 寫標題了
    try:
...
# ================= 執行模式 (V0.0.8 - Deep Research Edition) =================
...
def run_security_mode(config):
    ...
    # 1. 新聞快報 (TG) - 保持 V0.0.7 的生動中文懶人包
    print(" ↳ 發送新聞快報 (TG)...")
    news_prompt = f"""
    你是資安情報官。請整理以下 RSS 情報。
    【內容】{raw}
    【格式要求】
    1. **標題必須翻譯成流暢的繁體中文** (不要顯示英文標題)。
    2. 使用 **生動的 Emoji** (如 🚨, 🐛, ⚠️, 🕵️) 放在標題前。
    3. 分類為：🔥 **今日高危** / 🌏 **全球態勢**。
    4. 每一則新聞包含：標題、📅 事件背景(一句話)、🔗 原始連結。
    5. 嚴禁 Markdown 粗體。
    """
    send_telegram(token, model.generate_content(news_prompt).text)
    
    # 2. CISSP 微課程 (TG) - 保持 V0.0.7 的單點突破+名詞解釋
    ...
    # 3. MD 日報 (Log) - 這裡進行大改版：專業、詳盡、無 Emoji
    print(" ↳ 生成詳細 MD 日報 (Professional)...")
    file_prompt = f"""
    ...
    【報告結構】
    # 🛡️ SOC 資安戰情日報 ({today_str})
    
    ## 1. 👨‍💼 CISO 架構師戰略總結 (Executive Summary)
    - **威脅態勢綜述**：以高階視角分析今日威脅對企業營運的衝擊。
    - **戰略行動建議**：給予 C-Level 管理層的具體決策建議。
    ...
    ## 3. 🎯 深度技術攻防分析 (Deep Dive Analysis)
    - 請挑選 3-5 則重大事件，進行長篇幅的技術拆解。
    - 每個事件請包含：
        - **事件描述**
        - **技術原理 (CVE/TTPs)**：詳細解釋攻擊鏈。
        - **IOCs (入侵指標)**：如果文中沒提供，請列出可能的偵測特徵。
        - **緩解措施**：具體的修補指令或 WAF 規則建議。
        
    ## 4. 🔮 威脅趨勢與前瞻 (Future Outlook)
    - 獨立成段，分析駭客組織動向與未來一季的防禦重點。
    """
...
def run_finance_mode(pf_data, mode="finance"):
    ...
    if mode == "finance":
        ...
        ## 4. 📝 交易員日誌 (Trader's Journal)
        - 總結今日盤勢，並制定明日的具體交易計畫 (Entry/Exit points)。
        """
