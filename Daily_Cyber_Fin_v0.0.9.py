...
def run_security_mode(config):
    ...
    # 2. CISSP 微課程 (TG) - 單點深入 + 白話文解釋 (V0.0.1 靈魂)
    ...
    # 3. MD 日報 (Log) - 專為 NotebookLM 設計的詳盡版 (保留 Emoji)
...
def run_finance_mode(pf_data, mode="finance"):
    ...
    # 2. 策略分析與教學 (TG) - 恢復 V0.0.1 的教學靈魂 + 名詞解釋
    print(" ↳ 發送新聞與教學 (TG)...")
    strategy_prompt = f"""
    你是 CFO 與技術導師。
    【持股狀態】
    ...
    請挑選一支股票，依照以下格式教學：
    1. **標題**：今日 K 線實戰：(股票名稱)
    2. **核心名詞解釋**：(解釋本次教學用到的技術指標，如：什麼是黃金交叉？用白話文解釋)
    3. **目前型態**：(解釋均線、KD、背離等)
    ...
    # 3. MD 日報 (Log) - 財經版 For NotebookLM (表格 + ASCII)
    if mode == "finance":
        print(" ↳ 生成詳細 MD 日報 (For NotebookLM)...")
        file_prompt = f"""
