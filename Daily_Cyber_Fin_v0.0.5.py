...
def run_security_mode(config):
    ...
    4. **事件背景**：一句話解釋發生什麼事。
    5. 使用 Emoji 豐富排版，但**嚴禁使用 Markdown 粗體**。
    """
    send_telegram(token, model.generate_content(news_prompt).text)
    
    # 2. CISSP 微課程 (TG) - V0.0.5: 強制恢復「名詞解釋」
    print(" ↳ 發送 CISSP 微課程...")
    class_prompt = f"""
...
def run_finance_mode(pf_data, mode="finance"):
    ...
    # 3. MD 日報 (Log) - V0.0.5: ASCII Art 與 深度診斷
    if mode == "finance":
        print(" ↳ 生成詳細 MD 日報...")
        file_prompt = f"""
